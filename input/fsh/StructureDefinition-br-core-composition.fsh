...
* type ^short = "Tipo da Composition (fixo em 'Sumário Internacional do Paciente)"
...
* type.coding.version ^short = "Versão da terminologia LOINC - se relevante"
...
* subject.display ^short = "Referência ao paciente ou grupo deste atendimento"
* subject.display ^definition = "Referência ao paciente ou grupo deste atendimento"
...
* attester.party ^short = "Referência à quem atestou a Composition"
* attester.party ^definition = "Referência à quem atestou a Composition"
...
* custodian ^short = "Identifica a organização ou grupo responsável pela Composition"
* custodian ^definition = "Identifica a organização ou grupo responsável pela Composition"
* custodian only Reference(br-core-organization)
* relatesTo ^short = "Relações que esta Composition tem com outras composições ou recursos"
* relatesTo ^definition = "Relações que esta Composition tem com outras composições ou recursos"
* relatesTo.id ^short = "Identificador único para referência cruzada"
* relatesTo.id ^definition = "Identificador único para referência cruzada"
...
* relatesTo.code ^short = "Código da asssociação da Composition"
* relatesTo.code ^definition = "Código da asssociação da Composition"
...
* relatesTo.targetReference ^short = "Referência à Composition alvo"
* relatesTo.targetReference ^definition = "Referência à Composition alvo"
* relatesTo.targetReference only Reference(br-core-composition)
* event ^short = "O serviço especializado  como uma colonoscopia ou uma apendicectomia, sendo documentado" 
* event ^definition = "O serviço especializado  como uma colonoscopia ou uma apendicectomia, sendo documentado" 
* event.id ^short = "Identificador  único para o elemento dentro de um recurso (para referências internas). Este pode ser qualquer valor de string que não contenha espaços"
...
* event.code ^short = "Esta lista de códigos representa os principais atos clínicos, como uma colonoscopia ou uma apendicectomia, sendo documentados. Em alguns casos, o evento é inerente ao typeCode, como um 'Histórico e Relatório Físico', no qual o procedimento documentado é necessariamente um ato de 'Histórico e Físico'"
...
* event.period ^short = "O período de tempo coberto pela composition. Não há declaração de que a documentação seja uma representação completa para este período, apenas que documenta eventos durante esse tempo"
...
* section.focus ^short = "Referência ao recurso associado desta seção"
* section.focus ^definition = "Referência ao recurso associado desta seção"
...
* section.orderedBy ^short = "Ordem dos itens nas section.entry"
* section.orderedBy ^definition = "Ordem dos itens nas section.entry"
* section.orderedBy from http://hl7.org/fhir/ValueSet/list-order (preferred)
* section.entry ^short = "Uma referência ao recurso a partir do qual a narrativa da seção é derivada"
* section.entry ^definition = "Uma referência ao recurso a partir do qual a narrativa da seção é derivada"
* section.emptyReason ^short = "Explicação porquê a seção está vazia"
* section.emptyReason ^definition = "Explicação porquê a seção está vazia"
